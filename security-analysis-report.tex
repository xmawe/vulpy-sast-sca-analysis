...
\begin{titlepage}
    ...
    {\large Analyse SAST et SCA avec Jenkins, Bandit et Trivy\\[3cm]}
...
Ce rapport présente une analyse complète de sécurité du projet Vulpy, une application web Flask volontairement vulnérable, en utilisant des outils d'analyse statique (SAST) et d'analyse de composition logicielle (SCA). L'objectif est d'identifier, documenter et corriger les vulnérabilités présentes dans le code source et les dépendances.

\subsection*{Informations du Projet}
\begin{itemize}[leftmargin=*]
    \item \textbf{Lien du projet :} \url{https://github.com/xmawe/vulpy-sast-sca-analysis}
    
    \item \textbf{Rapports avant corrections :} \url{https://github.com/xmawe/vulpy-sast-sca-analysis/tree/main/reports/before-vul-correction}
    
    \item \textbf{Rapports après corrections :} \url{https://github.com/xmawe/vulpy-sast-sca-analysis/tree/main/reports/after-vul-correction}
\end{itemize}
...
Bandit 1.8.6 (SAST), Trivy 0.48.0 (SCA), Jenkins (CI/CD), Docker
...
Le pipeline exécute 10 étapes : SAST (Bandit), SCA (Trivy sur dépendances, secrets, supply chain), build Docker, et scan d'image.
...
Total : environ 13 fichiers par exécution du pipeline.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Corrections appliquées} : Élimination complète des 11 vulnérabilités détectées dans good/ (100\% de taux de correction)
...
Vulnérabilités détectées (bad/) & 22 (inchangées) \\
Vulnérabilités détectées (good/ avant) & 11 \\
Vulnérabilités HIGH corrigées & 2 (100\%) \\
Vulnérabilités MEDIUM corrigées & 5 (100\%) \\
Vulnérabilités LOW corrigées & 4 (100\%) \\
Vulnérabilités finales (good/ après) & \textcolor{green}{\textbf{0}} \\
Taux de correction & \textcolor{green}{\textbf{100\%}} \\
...
Rapports générés & 13 fichiers par scan \\
Étapes du pipeline & 14 stages (séparées bad/good) \\
Version Bandit utilisée & 1.8.6 \\
Version Trivy utilisée & 0.48.0 \\ \bottomrule
\end{tabular}
...
\begin{itemize}[leftmargin=*]
    \item Détection précoce des vulnérabilités dans le cycle de développement
    \item Correction complète de toutes les vulnérabilités dans la version good/ (taux de réussite : 100\%)
    ...
    \item Documentation exhaustive des vulnérabilités et corrections (11 vulnérabilités corrigées dans 7 fichiers)
...
\begin{enumerate}[leftmargin=*]
    \item Pre-commit hooks Git
    \item Tests DAST (Dynamic Application Security Testing)
    \item Alertes automatiques pour nouvelles CVE
    \item Formation sécurité de l'équipe
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Rapports avant corrections :}
    
    \url{https://github.com/xmawe/vulpy-sast-sca-analysis/tree/main/reports/before-vul-correction}
    
    Ces rapports montrent l'état initial avec 11 vulnérabilités dans good/
    
    \item \textbf{Rapports après corrections :}
    
    \url{https://github.com/xmawe/vulpy-sast-sca-analysis/tree/main/reports/after-vul-correction}
    
    Ces rapports confirment l'élimination complète des vulnérabilités (0 détecté)
